...
let compose_subs s1 s2 = IntMap.union (fun _ mty1 _ -> Some(mty1)) (IntMap.map (apply_m s1) s2) s1;;
...
let rec type_infer env t =
  ...
  |Lam(s, var, e) ->
    let env =
      ...
      |Self(f_var) ->
        ...
        VarMap.union (fun _ _ ty2 -> Some(ty2)) env' (VarMap.singleton f_var (TPoly([], TAlpha(alpha))))
      |_ -> env
    in
    let alpha = mk_alpha () in
    let env' = remove_env env var in
    let env'' = VarMap.union (fun _ _ ty2 -> Some(ty2)) env' (VarMap.singleton var (TPoly([], TAlpha(alpha)))) in
  ...
  |BeginMutual(_) -> raise Mutual_Def
  |EndMutual(_) -> raise Mutual_Def
;;
...
let type_term t =
  let _ = type_inference VarMap.empty t in
  t
;;
